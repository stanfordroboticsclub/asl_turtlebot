...
class AStar(object):
    ...
    def get_neighbors(self, x):
        ...
        currentX, currentY = x
        for x in range(-1, 2):
            xVal = x * self.resolution
            for y in range(-1, 2):
            # for yVal in range(-1*int(self.resolution), int(self.resolution + 1)):
                yVal = y * self.resolution
                tempX = currentX + xVal
                tempY = currentY + yVal
                tempTuple = (tempX, tempY)
                # if(tempX < 0 or tempY < 0 or (xVal == 0 and yVal == 0)):
                #     continue
                tempTuple = self.snap_to_grid(tempTuple)
                if(self.is_free(tempTuple)):
                    neighbors.append(tempTuple)
    ...
    def solve(self):
        ...
        print "Current Position: ", self.x_init
        print "goal position:", self.x_goal

        while len(self.open_set)>0:
            currentTuple = self.find_best_f_score()
            # print("current Tuple: ", currentTuple)

            dist = self.distance(currentTuple, self.x_goal)
            if dist < min_dist:
                min_dist = dist
                closest_reachable = currentTuple

            if(currentTuple == self.x_goal):
                self.path = self.reconstruct_path(self.x_goal)
                return self.path

            self.closed_set.append(currentTuple)
            self.open_set.remove(currentTuple)

            neighbors = self.get_neighbors(currentTuple)
            for tempTuple in neighbors:
                if(tempTuple in self.closed_set):
                    continue
                tempGScore = self.g_score[currentTuple] + self.distance(currentTuple, tempTuple) 
                
                if(not (tempTuple in self.open_set)):
                    self.open_set.append(tempTuple)
                elif(tempGScore > self.g_score[tempTuple]):
                    continue

                self.came_from[tempTuple] = currentTuple
                self.g_score[tempTuple] = tempGScore
                self.f_score[tempTuple] = tempGScore + self.distance(tempTuple, self.x_goal)
        
        # return False
        self.path = self.reconstruct_path(closest_reachable)
        return self.path
